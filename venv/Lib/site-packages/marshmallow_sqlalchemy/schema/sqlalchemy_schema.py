...
class SQLAlchemyAutoField(Field):
    def __init__(self, *, column_name=None, model=None, table=None, field_kwargs):
        ...
        if model and table:
            raise ValueError("Cannot pass both `models` and `table` options.")
...
class SQLAlchemySchemaOpts(LoadInstanceMixin.Opts, SchemaOpts):
    ...
    - ``models``: The SQLAlchemy models to generate the `Schema` from (mutually exclusive with ``table``).
    - ``table``: The SQLAlchemy table to generate the `Schema` from (mutually exclusive with ``models``).
    - ``load_instance``: Whether to load models instances.
    ...
    - ``transient``: Whether to load models instances in a transient state (effectively ignoring the session).
        Only relevant when ``load_instance`` is `True`.
    - ``model_converter``: `ModelConverter` class to use for converting the SQLAlchemy models to marshmallow fields.
    """

    def __init__(self, meta, *args, **kwargs):
        super().__init__(meta, *args, **kwargs)

        self.model = getattr(meta, "models", None)
        self.table = getattr(meta, "table", None)
        if self.model is not None and self.table is not None:
            raise ValueError("Cannot set both `models` and `table` options.")
...
class SQLAlchemySchema(
    ...
    """Schema for a SQLAlchemy models or table.
...
def auto_field(
    ...
    **kwargs,
    ...
    """Mark a field to autogenerate from a models or table.
    ...
    :param model: Model to generate the field from.
        If ``None``, uses ``models`` specified on ``class Meta``.
