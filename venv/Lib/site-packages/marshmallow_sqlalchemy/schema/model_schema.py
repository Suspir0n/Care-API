...
class ModelSchemaOpts(LoadInstanceMixin.Opts, ma.SchemaOpts):
    ...
    - ``models``: The SQLAlchemy models to generate the `Schema` from (required).
    ...
    - ``model_converter``: `ModelConverter` class to use for converting the SQLAlchemy models to
        marshmallow fields.
    """
...
class ModelSchema(LoadInstanceMixin.Schema, ma.Schema, metaclass=ModelSchemaMeta):
    """Base class for SQLAlchemy models-based Schemas.
