...
class NewProductModel(db.Model):
    ...
    user = db.relationship('UserModel', backref=backref('product', uselist=False))
    sub_category_fk = db.Column(db.String, ForeignKey('sub_category.uid'))
    sub_category = db.relationship('SubCategoryModel', backref=backref('product', uselist=False))
